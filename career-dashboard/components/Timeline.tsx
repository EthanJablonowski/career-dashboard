...
export default function Timeline() {
  // Sort all projects chronologically (most recent first)
  const allProjects = [...projects].sort((a, b) => {
    const parseDate = (dateStr: string) => {
      if (dateStr === 'Present') return 2026.0;
      const match = dateStr.match(/Q(\d) (\d{4})/);
      if (match) {
  ...
  const getTypeBadge = (nodeType: string) => {
    switch (nodeType) {
      case 'primary':
  ...
  return (
    <section className="max-w-4xl mx-auto px-6 py-12">
      ...
      <div className="relative">
        ...
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-slate-400 via-slate-300 to-slate-200 rounded-full" />
          ...
          {allProjects.map((project) => {
            ...
            return (
              <div key={project.id} className="relative pl-8">
                {/* Node dot */}
                <div className={`absolute left-0 top-2 w-3 h-3 rounded-full ${nodeColor} ring-4 ring-white shadow-sm`} style={{ transform: 'translateX(-5px)' }} />
                ...
                >
                  <div className="mb-2">
                    <div className="flex items-start justify-between gap-3 mb-1">
                      ...
                      <span className={`text-xs px-2.5 py-1 rounded-full border font-medium whitespace-nowrap ${badge.color}`}>
                        {badge.label}
                      </span>
                    </div>
